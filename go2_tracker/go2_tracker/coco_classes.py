#!/usr/bin/env python3
"""
COCO Class Mapping Utility

Maps class names to COCO class IDs for YOLO models.
"""

# COCO class names to IDs mapping
COCO_CLASSES = {
    "person": 0,
    "bicycle": 1,
    "car": 2,
...
COCO_IDS = {v: k for k, v in COCO_CLASSES.items()}
...
def get_class_id(class_name: str) -> int:
    ...
    return COCO_CLASSES.get(name, -1)
...
def get_class_name(class_id: int) -> str:
    ...
    """
    return COCO_IDS.get(class_id, "unknown")
...
def list_classes():
    ...
    print("-" * 40)
    for name, id in sorted(COCO_CLASSES.items(), key=lambda x: x[1]):
        print(f"  {id:2d}: {name}")
    print("-" * 40)
